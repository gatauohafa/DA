...
const CAT_LABEL = {
  model_tanpa_sku: 'model tanpa SKU', kelompok_tanpa_varian: 'kelompok tanpa varian', satuan_tak_valid: 'satuan tak valid',
...
function ResultCard({ result }) {
  return (
    <GlassCard className="p-4 text-xs text-emerald-300 space-y-1" data-testid="fill-result">
      ...
      <div>HPP: {result.models_hpp_applied} model dihitung · <span className="text-amber-300" data-testid="fill-result-hpp-unvalidated">{result.hpp_unvalidated || 0} model HPP belum tervalidasi</span> (material harga 0 / kemasan tanpa isi) · {result.hpp_validated || 0} tervalidasi</div>
      {result.scope !== 'bom' && <div>{result.materials_updated} material · {result.accounts_updated} rekening · {result.stores_updated} toko · {result.sku_prices_updated || 0} harga SKU · {result.opening_stock_rows || 0} stok awal · {result.salaries_updated || 0} gaji · {result.models_weight_updated || 0} berat model</div>}
  ...
  return (
    <div className="space-y-5" data-testid="fill-page">
      <PageHeader icon={FileSpreadsheet} eyebrow="Master · Pengisian Massal" title="Impor Harga · Satuan · Rekening · BOM"
        subtitle="Sesi go-live: unggah berkas terisi → pratinjau per kategori → Terapkan (hanya BOM). BOM aksesoris per (model, varian) yang ada di berkas diganti utuh; model lain tidak disentuh. Tidak ada tebakan warna — kelompok tanpa kolom varian pada model banyak-kelompok dilewati dan dicatat. Setelah itu unduh Laporan Sisa + berkas Data yang Perlu Diisi terbaru." />
      <div className="grid md:grid-cols-4 gap-4">
        <GlassCard className="p-5 space-y-3"><div className="text-[10px] uppercase text-muted-foreground font-semibold">Langkah 1</div><h3 className="font-semibold text-sm">Unduh berkas isian</h3>
          ...
          <p className="text-[11px] text-muted-foreground">Dibuat dari kondisi DB saat ini — setelah BOM diterapkan, angkanya sudah yang terbaru.</p>
